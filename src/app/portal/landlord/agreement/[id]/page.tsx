...
function LandlordSigningContent() {
  ...
  useEffect(() => {
    fetch(`/api/agreements/${id}/landlord-view?token=${token ?? ''}`)
      .then(r => r.json())
      .then(d => {
  ...
  async function handleSign() {
    ...
    try {
      const res = await fetch(`/api/agreements/${id}/sign-landlord`, {
        method: 'POST',
  ...
  if (data?.landlordSignedAt && !signed) {
    return (
      <div className="max-w-lg mx-auto mt-12 text-center">
        ...
        <p className="text-sm text-[#8a7968] mt-2">
          Your signature was recorded. Both parties have signed — the fully executed agreement has been emailed to you and the tenant.
        </p>
      </div>
    )
  }
  ...
  if (signed) {
    return (
      <div className="max-w-lg mx-auto mt-12 text-center">
        <CheckCircle className="w-14 h-14 text-green-500 mx-auto mb-4" />
        <h2 className="text-2xl font-bold text-[#1a1a1a] mb-2">Agreement Fully Signed</h2>
        <p className="text-[#8a7968]">
          Thank you. Both you and the tenant have signed the Assured Shorthold Tenancy Agreement.
          A copy of the fully executed agreement has been emailed to all parties.
        </p>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-[#1a1a1a]">Sign Tenancy Agreement</h1>
        <p className="text-sm text-[#8a7968] mt-1">
          {data?.propertyAddress ?? 'Please review the agreement below.'}
        </p>
      </div>
      ...
      {data?.tenantSignedName && (
        <div className="bg-green-50 border border-green-200 rounded-xl p-4 flex items-center gap-3">
          ...
          <p className="text-sm text-green-800">
            <strong>{data.tenantSignedName}</strong> (tenant) has already signed this agreement.
            Your countersignature will make it legally binding.
          </p>
        </div>
      )}
      ...
      {data && (
        <div className="bg-white border border-gray-200 rounded-xl p-5">
          <h3 className="font-semibold text-[#1a1a1a] mb-3">Agreement Summary</h3>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-5 space-y-4">
        ...
        <label className="flex items-start gap-3 cursor-pointer">
          ...
          <span className="text-sm text-[#404040]">
            I confirm that I have read and understood the full Assured Shorthold Tenancy Agreement, and I agree to let the property on the terms stated.
          </span>
        </label>
        ...
        <p className="text-xs text-[#8a7968] text-center">
          Once signed, both parties will receive the fully executed agreement by email. Your signature is legally binding.
        </p>
      </div>
    </div>
  )
