...
export default function ComplianceClient({ properties, items, certTypes }: Props) {
  ...
  async function runComplianceCheck() {
    ...
    setCheckResult(null)
    try {
      const res = await fetch('/api/cron/compliance', { method: 'POST' })
      const data = await res.json()
      if (!res.ok) throw new Error(data.error ?? 'Failed')
      setCheckResult(`Done — ${data.certsChecked} cert${data.certsChecked !== 1 ? 's' : ''} checked, ${data.rtrChecked} RTR checks, ${data.notificationsSent} alert${data.notificationsSent !== 1 ? 's' : ''} sent`)
      router.refresh()
    } catch (e) {
      setCheckResult(e instanceof Error ? e.message : 'Failed')
    } finally {
  ...
  return (
    <>
      <div className="flex items-center justify-end mb-4 gap-3">
        ...
        >
          {runningCheck ? <Loader2 size={14} className="animate-spin" /> : <Bell size={14} />}
          {runningCheck ? 'Checking…' : 'Run compliance check'}
        </button>
      </div>
      {checkResult && (
        <p className="text-sm text-center py-2 px-4 bg-amber-50 border border-amber-200 text-amber-700 rounded-lg mb-4">{checkResult}</p>
      )}
