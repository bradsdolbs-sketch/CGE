              ...
              <div className="p-5">
                ...
                {editingId !== l.id && (
                  <div className="flex flex-wrap items-center gap-2 mt-4 pt-3 border-t border-gray-100">
                    {l.availableFrom && (
                      <span className="inline-flex items-center gap-1 text-xs font-medium bg-[#1A3D2B]/8 text-[#1A3D2B] px-2 py-0.5 rounded-full">
                        Available {format(new Date(l.availableFrom), 'd MMM yyyy')}
                      </span>
                    )}
                    {l.furnished && (
                      <span className="inline-flex items-center text-xs text-gray-500 bg-gray-100 px-2 py-0.5 rounded-full">Furnished</span>
                    )}
                    {l.billsIncluded && (
                      <span className="inline-flex items-center text-xs text-gray-500 bg-gray-100 px-2 py-0.5 rounded-full">Bills incl.</span>
                    )}
                    {l.parking && (
                      <span className="inline-flex items-center text-xs text-gray-500 bg-gray-100 px-2 py-0.5 rounded-full">Parking</span>
                    )}
                    {l.petsAllowed && (
                      <span className="inline-flex items-center text-xs text-gray-500 bg-gray-100 px-2 py-0.5 rounded-full">Pets OK</span>
                    )}
                    {!l.availableFrom && !l.furnished && !l.billsIncluded && !l.parking && !l.petsAllowed && (
                      <span className="text-xs text-gray-400 italic">No features set</span>
                    )}
                    <span className="ml-auto text-xs text-gray-400">Updated {format(new Date(l.updatedAt), 'd MMM yyyy')}</span>
                  </div>
                )}
              </div>
            </div>
