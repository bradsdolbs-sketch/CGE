...
export default function TenanciesClient({ tenancies }: { tenancies: TenancyRow[] }) {
  ...
  async function sendRenewalAlerts() {
    ...
    try {
      ...
      setAlertResult(`Done — ${data.processed} tenancy${data.processed !== 1 ? 's' : ''} checked, ${data.emailsSent} emails sent`)
      router.refresh()
    } catch (e) {
      setAlertResult(e instanceof Error ? e.message : 'Failed')
    } finally {
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap gap-3 items-center">
        ...
        <button
          ...
          className="ml-auto flex items-center gap-2 bg-amber-500 hover:bg-amber-600 text-white text-sm font-medium px-4 py-2 rounded-lg transition disabled:opacity-50"
        >
          {sendingAlerts ? <Loader2 size={14} className="animate-spin" /> : <Bell size={14} />}
          {sendingAlerts ? 'Checking…' : 'Send renewal alerts'}
        </button>
      </div>
      {alertResult && (
        <p className="text-sm text-center py-2 px-4 bg-amber-50 border border-amber-200 text-amber-700 rounded-lg">{alertResult}</p>
      )}
